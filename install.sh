...
Run protocol:
1) Read $LOOP_QUEUE_FILE first, then $LOOP_INBOX_FILE, then project context docs.
2) Pick ONE highest-impact unblocked task.
3) Execute one atomic step (8-20 min).
4) Run relevant verification (tests/lint/build/smoke).
5) Write result + next step to memory and queue docs.
6) Report concise summary with changed files and verification.
...
Run protocol:
1) Scan roadmap/backlog/docs (ROADMAP, TODO, IDEAS, issues, TODO/FIXME).
2) Select ONE feature extension or quality enhancement with highest impact.
3) Define acceptance criteria and smallest next increment.
4) Append proposal to $LOOP_INBOX_FILE using this contract:
   - id: AUTOCLAW-YYYYMMDD-HHMM
   - title:
   - type: feature|quality|research|tech-debt
   - impact: high|medium|low
   ...
   - owner: ralphclaw
   - status: ready
5) Promote highest-impact ready item into $LOOP_QUEUE_FILE with priority (P0/P1/P2).
6) Implement one bounded increment end-to-end.
7) Run verification and document outcome + next increment.
...
configure_autonomous_loops() {
  local ralphclaw_job_name="Kai RalphClaw (15m)"
  local autoclaw_job_name="Kai AutoClaw (3h)"
  local watchdog_job_name="Kai RalphClaw Watchdog (20m)"
  local kpi_job_name="Kai Loop KPI Weekly"
